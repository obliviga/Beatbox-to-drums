...
2. **Deploy the relay**: browsers can't call `api.stability.ai` directly (no CORS), so you run your own tiny forwarder. On [dash.cloudflare.com](https://dash.cloudflare.com) → *Workers & Pages* → *Create Worker* (the Hello World starter), paste the contents of [`worker/relay.js`](worker/relay.js), deploy — the free tier is far more than enough. The worker only forwards the one endpoint the app uses. *(Alternative, no copy-paste: use Cloudflare's "Import a repository" flow on this repo with project name `beatbox-relay`, empty build command, and deploy command `npx wrangler deploy` — the repo's [`wrangler.jsonc`](wrangler.jsonc) points it at the relay.)*
...
After setup, **✨ is one tap = one track** (15–40 s); the **⚙ AI settings** link underneath reopens the panel to change the style prompt — try "boom bap drum break, dusty vinyl" or "cinematic percussion, huge room". Every tap generates a fresh track, so re-roll until one hits. Your settings persist in the browser, so this is a one-time entry per device. *(Developing locally? Copy [`js/local-config.example.js`](js/local-config.example.js) to `js/local-config.js` — gitignored — to pre-fill these fields without committing anything.)*
